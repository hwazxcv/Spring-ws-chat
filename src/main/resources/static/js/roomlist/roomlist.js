...
$(function () {
    ...
    // 이벤트창이 열릴 때 이벤트 처리 => roomNumber 가져오기
    $("#enterRoomModal").on("show.bs.modal", function (event) {
        console.log(event);
        roomNumber = $(event.relatedTarget).data('id');
        console.log("roomNumber: " + roomNumber);
    });

    $("#confirmPwdModal").on("show.bs.modal", function (e) {
        roomNumber = $(e.relatedTarget).data('id');
         console.log("roomNumber: " + roomNumber);
    });
...
function confirmPWD() {
    $("#confirmPwd").on("keyup", function () {
        ...
        let url = '/chat/confirmPwd/' + roomNumber
        let data = {
                "roomPwd": $confirmPwd
        };
...
function enterRoom() {
    ...
    let successCallback = function (result) {
         console.log("동작완료")
         console.log("확인 : "+chkRoomUserCnt(roomNumber))

        if (result) {
            chkRoomUserCnt(roomNumber);
...
// 채팅방 삭제
function delRoom() {
    let url = "/chat/delRoom/" + roomNumber;
    let successCallback = function (result) {
        alert("방 삭제를 완료했습니다");
        location.href = "/";
    };
...
function chkRoomUserCnt(roomNumber) {
    let url = '/chat/chkUserCnt/' + roomNumber;
    let successCallback = function (result) {
        if (!result) {
        ...
        location.href = '/chat/room?roomNumber=' + roomNumber;
    };
    let errorCallback = function (error) {
        console.error(error);
    }
